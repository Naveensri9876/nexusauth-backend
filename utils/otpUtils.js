    ...
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a1a; color: #fff; border-radius: 16px; overflow: hidden;">
        ...
        <div style="padding: 40px; text-align: center;">
          ...
          <p style="color: #aaa; margin-bottom: 32px;">Use the code below to verify your email address. It expires in <strong style="color: #3ecfcf;">5 minutes</strong>.</p>
          <div style="background: linear-gradient(135deg, #1a1a2e, #16213e); border: 2px solid #6c63ff; border-radius: 12px; padding: 24px; display: inline-block;">
            <p style="margin: 0; font-size: 48px; font-weight: 900; letter-spacing: 12px; color: #6c63ff; font-family: monospace;">${otp}</p>
          </div>
          <p style="color: #555; margin-top: 32px; font-size: 12px;">If you didn't request this, please ignore this email.</p>
        </div>
      </div>
    `
  };
...
const sendPhoneOTP = async (phone, otp) => {
  if (process.env.USE_FAKE_OTP === 'true') {
    // FAKE OTP - just log to console (no Twilio needed)
